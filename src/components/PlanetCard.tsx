...
export const PlanetCard: React.FC<PlanetCardProps> = ({ planetName, system, sites, index, planetData, resourceTypes }) => {
  const [isHovered, setIsHovered] = useState(false);
  const isAssemblyPlanet = sites.some(s => s.isAssemblyHub);

  return (
    <div
      className={`glass-panel rounded-xl p-4 nasa-accent hover:border-sky-500/40 transition-all duration-300 relative overflow-visible ${isAssemblyPlanet ? 'border-amber-500/40 ring-1 ring-amber-500/10' : ''
        }`}
          ...
          <div className="glass-panel p-4 rounded-xl border border-sky-500/30 bg-slate-950/95 shadow-2xl backdrop-blur-xl">
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              ...
              <div className="col-span-2 border-t border-white/5 pt-2 mt-1">
                {(() => {
                  const inorganic = (planetData.resources || []).filter((r: string) => resourceTypes?.[r] === 'inorganic');
                  const organic = (planetData.resources || []).filter((r: string) => resourceTypes?.[r] === 'organic');

                  return (
                    <div className="space-y-2">
                      {inorganic.length > 0 && (
                        <div>
                          <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest mb-1 leading-none text-center">Inorganic Resources</p>
                          <div className="flex flex-wrap gap-1 justify-center">
                            {inorganic.map((res: string) => (
                              <span key={res} className="text-[7px] bg-slate-800 text-slate-400 px-1 py-0.5 rounded font-mono border border-slate-700">{res}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {organic.length > 0 && (
                        <div>
                          <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest mb-1 leading-none text-center">Organic Resources</p>
                          <div className="flex flex-wrap gap-1 justify-center">
                            {organic.map((res: string) => (
                              <span key={res} className="text-[7px] bg-emerald-900/30 text-emerald-400 px-1 py-0.5 rounded font-mono border border-emerald-500/20">{res}</span>
                            ))}
                          </div>
                        </div>
                      )}
              ...
              <div className="p-3 border-b border-white/5">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black ${site.isAssemblyHub ? 'bg-amber-500 text-slate-950' : 'bg-sky-500/20 text-sky-400 border border-sky-500/30'
                      }`}>
                      ...
                      <div key={lIdx} className={`rounded p-1.5 border border-white/5 flex flex-col gap-0.5 bg-slate-950/50 transition-colors ${isOverloaded ? 'border-red-500/30' : ''}`}>
                        <div className="flex justify-between items-center text-[7px] font-black uppercase tracking-tighter">
                          <div className="flex gap-1.5 items-center">
                            ...
                            <span className={`px-1 rounded-sm border ${isOverloaded ? 'bg-red-500/20 text-red-400 border-red-500/30' :
